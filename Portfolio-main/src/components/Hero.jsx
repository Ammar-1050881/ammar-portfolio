...
import heroVideo from '../assets/hero video/ammar-talking.mp4';
import { heroContent, personalInfo, socialLinks } from '../data/portfolioData';

// 1. Added hasEntered prop right here
const Hero = ({ hasEntered }) => {
  ...
  // 2. The trigger that plays the video exactly when the curtains open!
  useEffect(() => {
    if (hasEntered && videoRef.current) {
      videoRef.current.muted = false; // Ensure we override any mute
      setIsMuted(false);
      videoRef.current.volume = 1;    // 100% volume
  ...
  // 3. Flips the UI button back to "Play Reel" when the video naturally finishes
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <video
        ref={videoRef}
        // Removed `loop` so it stops perfectly after one play
        muted={isMuted}
        playsInline
        preload="auto"
        onEnded={handleVideoEnd} // Triggers when the video is over
        className="absolute top-0 left-0 w-full h-full object-cover z-0"
      >
        <source src={`${heroVideo}#t=0.1`} type="video/mp4" />
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          {/* Main Heading with Layout Readability Fixes */}
          <h1 
            data-aos="fade-up"
            className="text-white text-3xl md:text-5xl font-bold mb-4 tracking-tight drop-shadow-[0_4px_12px_rgba(0,0,0,0.95)]"
          >
            {heroContent.greeting}, <br /> 
            <span className="text-[#FFD343] block mt-2 drop-shadow-[0_4px_10px_rgba(0,0,0,0.9)]">
          ...
          <p 
            data-aos="fade-up"
            data-aos-delay="200"
            className="text-white text-sm md:text-lg font-semibold mb-8 max-w-md drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]"
          >
          ...
          >
            ...
            <a 
              ...
              className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-black/50 border border-white/30 text-white font-semibold hover:bg-black/70 transition-all duration-300 backdrop-blur-md shadow-lg"
            >
            ...
            <a 
              ...
              download
              className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-black/50 border border-white/30 text-white font-semibold hover:bg-black/70 transition-all duration-300 backdrop-blur-md shadow-lg flex items-center gap-2"
            >
